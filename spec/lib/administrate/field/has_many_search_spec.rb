require "administrate/field/has_many_search"

RSpec.describe Administrate::Field::HasManySearch do
  let(:page) { :edit }
  let(:post) { create(:post) }
  let(:field) { Administrate::Field::HasManySearch.new(:author, post.author, page) }

  describe "#to_partial_path" do
    it "returns a partial based on the page being rendered" do
      expect(field.to_partial_path).to eq("/fields/has_many_search/#{page}")
    end
  end
  ...
  describe "#associated_class" do
    ...
    context "when association data is nil" do
      let(:field) { Administrate::Field::HasManySearch.new(:author, nil, page) }
